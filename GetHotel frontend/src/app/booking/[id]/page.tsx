...
import { hotelApi, bookingApi, paymentApi, messageApi } from "@/lib/api";
...
function BookingContent() {
    ...
    // Parse multiple rooms from URL: room_ID=QTY
    const selectedRoomsInfo = Array.from(searchParams.entries())
        .filter(([key]) => key.startsWith('room_'))
        .map(([key, val]) => ({ id: key.replace('room_', ''), quantity: parseInt(val) }));
        ...
        const fetchData = async () => {
            try {
                ...
                if (selectedRoomsInfo.length > 0) {
                    ...
                    const allRooms = hotelRes.data.room || hotelRes.data.rooms || [];
                    const filtered = allRooms.filter((r: any) => roomIds.includes(r.id.toString()));
                    setSelectedRoomsData(filtered);
                } else {
                    setSelectedRoomsData([hotelRes.data.room?.[0] || hotelRes.data.rooms?.[0]]);
                }
    ...
    let subtotal = validRooms.reduce((sum, room) => {
        ...
        return sum + ((room.pricePerNight || 0) * info.quantity * nights);
    }, 0);

    // FALLBACK: If no specific rooms found/selected, use hotel's base price
    if (subtotal === 0 && hotel) {
        subtotal = (hotel.pricePerNight || 0) * nights;
    }

    const total = subtotal;
    const taxes = Math.round(total * 0.107); // Representing the ~12% portion that is already included
    const pricePerNight = total / (nights || 1);
            ...
            <div className="max-w-6xl mx-auto px-6 -mt-12 pb-24">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    <div className="lg:col-span-2 space-y-10">
                        {/* Hotel & Dates Card */}
                        <section className="bg-white rounded-[40px] p-8 md:p-12 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] border border-slate-50">
                        ...
                        <section className="bg-white rounded-[40px] p-8 md:p-12 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] border border-slate-50">
                            ...
                            <div className="space-y-6 mb-12">
                                {selectedRoomsData.map((room, idx) => (
                                    <div key={idx} className="flex justify-between items-center p-6 bg-slate-50 rounded-3xl">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-slate-900 font-black border border-slate-100 shadow-sm">
                                                {selectedRoomsInfo[idx]?.quantity || 1}
                                            </div>
                                            <div>
                                        ...
                                        <div className="text-right">
                                            <p className="text-sm font-black text-slate-900">{formatPrice(room?.pricePerNight)}</p>
                                            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">per night</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="relative">
                        <div className="sticky top-12 space-y-8">
                            <div className="bg-white rounded-[40px] p-8 md:p-10 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] border border-slate-50 relative overflow-hidden">
                                ...
                                <div className="space-y-6 mb-10">
                                    <div className="flex justify-between items-center">
                                        <span className="text-slate-500 font-bold text-sm uppercase tracking-widest">Total Amount</span>
                                        <span className="text-xl font-black text-slate-900">{formatPrice(total)}</span>
                                    </div>
                                    <div className="flex justify-between items-center pt-6 border-t border-slate-100">
                                        <div className="flex items-center gap-2">
                                            <div className="w-2 h-2 bg-emerald-500 rounded-full" />
                                            <span className="text-emerald-600 font-black text-[10px] uppercase tracking-widest">Platform Fee (Paid)</span>
                                        </div>
                                        <span className="text-base font-black text-emerald-600 italic">{formatPrice(Math.round(total * 0.18))}</span>
                                    </div>
                                    <div className="flex justify-between items-center pt-4">
                                        <div className="flex items-center gap-2">
                                            <div className="w-2 h-2 bg-brand-500 rounded-full" />
                                            <span className="text-slate-600 font-black text-[10px] uppercase tracking-widest">Pay at Hotel</span>
                                        </div>
                                        <span className="text-lg font-black text-slate-900 italic">{formatPrice(total - Math.round(total * 0.18))}</span>
                                    </div>
                                </div>
                                ...
                                <div className="p-6 bg-slate-50 rounded-3xl mb-8 border border-slate-100">
                                    ...
                                    <p className="text-[9px] text-slate-400 font-bold uppercase leading-relaxed">
                                        Your payment of {formatPrice(Math.round(total * 0.18))} has been processed successfully. Please show this receipt at the front desk.
                                    </p>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <button 
                                        ...
                                        className="w-full py-5 bg-slate-900 text-white font-black rounded-3xl hover:bg-black transition-all shadow-xl shadow-slate-200 text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3"
                                    >
                                    ...
                                    <button 
                                        ...
                                        className="w-full py-5 bg-white border-2 border-slate-100 text-slate-900 font-black rounded-3xl hover:border-brand-200 transition-all text-xs uppercase tracking-[0.2em]"
                                    >
                                        Go to My Bookings
                                    </button>
                                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        <section className="bg-white rounded-[40px] p-8 md:p-10 shadow-xl shadow-slate-200/50 border border-white">
                            <h2 className="text-2xl font-black text-slate-900 mb-8 italic uppercase tracking-tight">Review Your Selection</h2>
                            ...
                            <div className="space-y-6">
                                {selectedRoomsData
                                    ...
                                    .map((room, idx) => {
                                        const info = selectedRoomsInfo.find(i => i.id === room.id.toString()) || { quantity: 1 };
                                    return (
                                        <div key={room.id} className="flex flex-col md:flex-row gap-6 p-6 bg-slate-50/50 rounded-3xl border border-slate-100 relative overflow-hidden group">
                                            ...
                                            <div className="flex-1">
                                                <div className="flex justify-between items-start">
                                                    ...
                                                    <div className="bg-white px-3 py-1.5 rounded-xl border border-slate-200 text-center">
                                                        ...
                                                        <p className="text-sm font-black text-slate-900">{info.quantity}</p>
                                                    </div>
                                                </div>
                                                <div className="mt-4 flex items-center gap-4">
                                                    ...
                                                    <div className="flex items-center gap-1.5 text-blue-600">
                                                        <Check className="w-3.5 h-3.5" />
                                                        <span className="text-[10px] font-black uppercase tracking-widest">Breakfast Included</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <section className="bg-white rounded-[40px] p-8 md:p-10 shadow-xl shadow-slate-200/50 border border-white">
                            <h2 className="text-2xl font-black text-slate-900 mb-8 italic uppercase tracking-tight">Guest Information</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <input 
                                        ...
                                        className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold focus:bg-white focus:border-brand-600 outline-none transition-all"
                                    />
                                </div>
                                <div className="space-y-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        <select 
                                            ...
                                            className="w-24 px-3 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold focus:bg-white focus:border-brand-600 outline-none transition-all appearance-none cursor-pointer"
                                        >
                                        ...
                                        <input 
                                            ...
                                            className="flex-1 px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold focus:bg-white focus:border-brand-600 outline-none transition-all"
                                        />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mt-8 space-y-2">
                                ...
                                <textarea 
                                    ...
                                    className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold focus:bg-white focus:border-brand-600 outline-none transition-all min-h-[100px] resize-none"
                                />
                        ...
                        <div className="sticky top-28 space-y-6">
                            <div className="bg-white rounded-[40px] p-8 shadow-2xl shadow-brand-500/10 border border-white relative overflow-hidden">
                                ...
                                <div className="space-y-4 mb-8">
                                    ...
                                    <div className="flex justify-between items-center text-sm">
                                        <span className="text-slate-500 font-bold">Taxes & Fees (All-inclusive)</span>
                                        <span className="text-emerald-600 font-black italic uppercase tracking-widest text-[10px]">Included</span>
                                    </div>
                                    <div className="pt-4 border-t border-slate-100 space-y-4">
                                        <div className="flex justify-between items-center">
                                            <span className="text-sm font-black text-slate-900 uppercase tracking-tighter">All-inclusive Total</span>
