            ...
            <div className="max-w-7xl mx-auto px-6">
                ...
                <div className="flex gap-6 pb-12 overflow-x-auto snap-x snap-mandatory no-scrollbar">
                    {DESTINATIONS.map((dest, i) => (
                        <div key={dest.name} className="min-w-[240px] md:min-w-[300px] snap-start">
                            <Link href={`/hotels?city=${dest.name}`} className="group block relative aspect-[4/5] rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500">
                                <img
                                    src={dest.image}
                                    alt={dest.name}
